...
const AgendaView: React.FC<AgendaViewProps> = ({ selectedDate, events }) => {
  ...
  return (
    <div style={{ maxWidth: 500, margin: '0 auto' }}>
      ...
      {dayEvents.map(ev => (
        <div key={ev.id} style={{ background: '#4F8CFF', color: '#fff', borderRadius: 10, padding: '10px 16px', marginBottom: 10, fontSize: 15, fontWeight: 500, boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
          <div>{ev.title}</div>
          <div style={{ fontSize: 13, color: '#eaf3ff', marginTop: 2 }}>{ev.time || 'All day'}</div>
        </div>
      ))}
